...
| S-4 | 收到券商材料（PDF/Word/ZIP/TXT）時兩階段處理：【接收】每收到一個檔案，立即下載至 state/broker_queue/ 目錄並回報「📥 已排隊：{檔名}」，不做其他處理，繼續 polling 接收下一份；【處理】Kai 說「處理券商報告」後，執行 python3 skills/broker-materials/scripts/batch_process.py 逐一處理佇列中所有檔案，每份完成後立即回報分類結果，全批完成後清空佇列並回報「完成：成功 X 份 / 失敗 Y 份」，列出失敗原因。 |
