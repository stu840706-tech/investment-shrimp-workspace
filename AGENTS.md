...
| S-4 | 收到文件通知時（gateway 提供 /tmp 下本地路徑）：執行 mv <路徑> ~/.openclaw/workspace/state/broker_queue/ 並回報「📥 已排隊：{檔名}」，不做其他處理。當 Kai 說「處理券商報告」時：①python3 skills/broker-materials/scripts/collect_broker_files.py（補掃 /tmp 未移入的檔案）→ ②python3 skills/broker-materials/scripts/batch_process.py → ③完成後回報成功/失敗統計。 |
