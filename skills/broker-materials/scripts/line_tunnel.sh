...
LINE_TOKEN="$(tr -d '[:space:]' < "$HOME/.openclaw/line_token.txt" 2>/dev/null)"
: > "$LOG"
...
for i in $(seq 1 5); do
    CODE=$(curl -s -o /dev/null -w '%{http_code}' -X PUT https://api.line.me/v2/bot/channel/webhook/endpoint \
